          ...
          <div className="bg-coral-red flex flex-col justify-center items-center lg:w-1/5 sm:w-4/5 xsm:w-3/5 mt-2 rounded-lg">
            <>
              {overallTypes.map((type, index) => {
                return (
                  <button key={index} onClick={() => filterStacks(type)} className="bg-orange-700 w-1/2 rounded-full my-2 font-palanquin text-white">
                    {type}
                  </button>
                );
              })}
              <button onClick={() => setIsFiltered2(false)} className="bg-orange-700 w-1/2 rounded-full my-2 font-semibold font-palanquin text-white">all</button>
            </>
          </div>
        )}
      </>
